...
function chaincodeInvoke() {
  ...
  # start=$(date +%s.%N)
  # for i in {1..50}; do
  # fcn_call='{"function":"'enrollItss'","Args":["'$((i+10))'","07/10/2023","'$i'","02102023", "[]"]}' 
  # peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  # end=$(date +%s.%N)
  # dt=$(echo "$end - $start" | bc)
  # dd=$(echo "$dt/86400" | bc)
  ...
  start=$(date +%s.%N)
  for i in {1..100}; do
  fcn_call='{"function":"'enrollItss'","Args":["'$((i+60))'","07/10/2023","'$i'","08102023", "[]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  # start=$(date +%s.%N)
  # for i in {1..250}; do
  # fcn_call='{"function":"'enrollItss'","Args":["'$((i+160))'","07/10/2023","'$i'","09102023", "[]"]}' 
  # peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  # end=$(date +%s.%N)
  # dt=$(echo "$end - $start" | bc)
  # dd=$(echo "$dt/86400" | bc)
  ...
  # start=$(date +%s.%N)
  # for i in {1..500}; do
  # fcn_call='{"function":"'enrollItss'","Args":["'$((i+410))'","07/10/2023","'$i'","05102023", "[]"]}' 
  # peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  # end=$(date +%s.%N)
  # dt=$(echo "$end - $start" | bc)
  # dd=$(echo "$dt/86400" | bc)
