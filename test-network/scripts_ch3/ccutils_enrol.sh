...
function chaincodeInvoke() {
  ...
  start=$(date +%s.%N)
  for i in {1..50}; do
  fcn_call='{"function":"'enrollItss'","Args":["'$((i+1010))'","07/10/2023","'$i'","03302023", "[]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  start=$(date +%s.%N)
  for i in {1..100}; do
  fcn_call='{"function":"'enrollItss'","Args":["'$((i+1060))'","07/10/2023","'$i'","04402023", "[]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  echo "Runtime in hours was $dh"
  echo "Runtime in minutes was $dm"
  echo "Runtime in seconds for 100 enrollment transaction  was $ds"
  #sleep 5
  
  start=$(date +%s.%N)
  for i in {1..250}; do
  fcn_call='{"function":"'enrollItss'","Args":["'$((i+1160))'","07/10/2023","'$i'","05502023", "[]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  echo "Runtime in hours was $dh"
  echo "Runtime in minutes was $dm"
  echo "Runtime in seconds for 250 enrollment transaction  was $ds"
  
  start=$(date +%s.%N)
  for i in {1..500}; do
  fcn_call='{"function":"'enrollItss'","Args":["'$((i+1410))'","07/10/2023","'$i'","06602023", "[]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  echo "Runtime in hours was $dh"
  echo "Runtime in minutes was $dm"
  echo "Runtime in seconds for 500 enrollment transaction  was $ds"
