...
function chaincodeInvoke() {
  ...
  start=$(date +%s.%N)
  for i in {1..50}; do
  fcn_call='{"function":"'retrieveValidationResponse'","Args":["0882030405060708","5992030785060713", "'$((i+1010))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..100}; do
  fcn_call='{"function":"'retrieveValidationResponse'","Args":["0882030405060708","5992030785060713", "'$((i+1060))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..250}; do
  fcn_call='{"function":"'retrieveValidationResponse'","Args":["0882030405060708","5992030785060713", "'$((i+1160))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..500}; do
  fcn_call='{"function":"'retrieveValidationResponse'","Args":["0882030405060708","5992030785060713", "'$((i+1410))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
...
function chaincodeInvokeEx() {
  ...
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1000))'", "'$((i+1010))'", "'$((i+1000))'"]}' 
  ...
  start=$(date +%s.%N)
  for i in {1..50}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1010))'", "'$((i+1020))'", "'$((i+1010))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..100}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1060))'", "'$((i+1080))'", "'$((i+1060))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..250}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1160))'", "'$((i+1180))'", "'$((i+1160))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..500}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1410))'", "'$((i+1430))'", "'$((i+1410))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
...
function chaincodeInvokeExmore() {
  ...
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1000))'", "'$((i+1015))'", "'$((i+1000))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..50}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1010))'", "'$((i+1025))'", "'$((i+1010))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..100}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+160))'", "'$((i+1085))'", "'$((i+160))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..250}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1160))'", "'$((i+1185))'", "'$((i+1160))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  ...
  start=$(date +%s.%N)
  for i in {1..500}; do
  fcn_call='{"function":"'uploadAuthorizationInfo'","Args":["0882030405060708","5992030785060713", "'$((i+1410))'", "'$((i+1435))'", "'$((i+1410))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
...
function chaincodeInvokeRevoke() {
  ...
  start=$(date +%s.%N)
  for i in {1..2}; do
  #fcn_call='{"function":"'initiateRevocation'","Args":["'$((i+10))'"]}' 
  fcn_call='{"function":"'initiateRevocation'","Args":["'$((i+23))'"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
