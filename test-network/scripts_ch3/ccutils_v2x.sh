...
function chaincodeInvoke() {
  ...
  start=$(date +%s.%N)
  for i in {1..50}; do
  fcn_call='{"function":"'uploadItssV2xInfo'","Args":["'$i'","03302023","[36,37,38,39]", "[Luxmbourge]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  start=$(date +%s.%N)
  for i in {1..100}; do
  fcn_call='{"function":"'uploadItssV2xInfo'","Args":["'$i'","04402023","[36,37,38,39]", "[Luxmbourge]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  start=$(date +%s.%N)
  for i in {1..250}; do
  fcn_call='{"function":"'uploadItssV2xInfo'","Args":["'$i'","05502023","[36,37,38,39]", "[Luxmbourge]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  ...
  start=$(date +%s.%N)
  for i in {1..500}; do
  fcn_call='{"function":"'uploadItssV2xInfo'","Args":["'$i'","06602023","[36,37,38,39]", "[Luxmbourge]"]}' 
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}"  -c "${fcn_call}" >&log.txt; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
...
function chaincodeInvokeEx() {
  ...
  start=$(date +%s.%N)
  for i in {1..2}; do
  peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["QueryAssets","{\"selector\":{\"id\":\"'$i'\" ,\"status\":\"OK\" },\"use_index\":[\"_design/v2xIndexDoc\", \"v2xIndex\"]}"]}'; done
  end=$(date +%s.%N)
  dt=$(echo "$end - $start" | bc)
  dd=$(echo "$dt/86400" | bc)
  dt2=$(echo "$dt-86400*$dd" | bc)
